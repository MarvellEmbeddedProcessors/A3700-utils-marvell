...
DDRPRSET=`$GETDDRPARAMS $DDRTOPFILE ddr_preset_conf`
if [ -z "$DDRPRSET" ]; then
	...
	# using preset ddr config file to instead of generated by ddr_tool
	cp $CLOCKSPATH/$DDRPRSET $CLOCKSPATH/ddr_static.txt
fi
